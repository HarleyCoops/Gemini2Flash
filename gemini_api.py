        ...
        for attempt in range(self.max_retries):
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(self.api_url, json=payload, timeout=20, headers=self.headers) as response:
...
class GeminiAPIWrapper:
    ...
    def _make_api_request(self, payload):
        """
        Makes the API request with retry logic.  Private method.
        ...
        for attempt in range(self.max_retries):
            try:
                response = requests.post(self.api_url, json=payload, timeout=20) # Added timeout for robustness
                response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
    ...
    def call_gemini_api(self, prompt, tools=None, temperature=0.0, top_p=1.0, top_k=1, max_output_tokens=200): #tuned for flash. Added tuning parameters
